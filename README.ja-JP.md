...
DG-LAB `.pulse` 波形を検査、プレビュー、編集、エクスポートするオープンソースのワークベンチです。
...
Electron アプリをビルドして起動します。現在のプラットフォーム向けインストーラーとポータブルアーカイブを作成するには、次を実行します。
...
2. 最初に Diagnostics を確認します。エラーがある文書はプレビューやエクスポートへ進めません。
3. タイムラインを再生またはポイントを選択し、エディターで波形を調整します。
4. Compare、Undo、Redo で変更を確認します。
5. `.pulse`、QR 画像、または波形プレビューをエクスポートします。入力バイトを保持する場合は Source
...
- 3 個を超える section も解析できますが、公式 App との相互運用性は未確認です。QR エクスポートは最大 3
  section です。
...
元ファイルは必ず保管し、エクスポートした波形を対象 App で確認してください。フォーマット調査と既知の境界は
