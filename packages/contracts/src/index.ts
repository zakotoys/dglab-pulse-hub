...
export const diagnosticSchema = z.object({
  ...
  parameters: z.record(
    z.string(),
    z.union([z.string(), finiteNumberSchema, z.boolean()])
  ).optional()
}).strict();
...
export const pulseGlobalsSchema = z.object({
...
export const pulseMetadataSchema = z.object({
...
export const streamPointSourceSchema = z.object({
  sectionIndex: nonNegativeIntegerSchema,
...
export const waveformPointSchema = z.object({
...
export const waveformStreamSchema = z.object({
...
export const pulseMetadataBundleSchema = z.object({
  ...
  if (value.pulse.enabledSectionCount + value.pulse.disabledSectionCount !== value.pulse.sectionCount) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
...
export const inspectDataSchema = z.object({
  ...
  const epsilon = 1e-6;
  if (value.pulse.sectionCount !== value.metadata.pulse.sectionCount ||
  ...
  if (value.recognition.ruleVersion !== value.pulse.ruleVersion ||
  ...
  if (value.stream !== null) {
    if (value.stream.digest.length !== 16 ||
        value.metadata.stream.stats.pointCount !== value.stream.points.length ||
        Math.abs(value.metadata.stream.stats.totalDurationMs - value.stream.totalDurationMs) > 1e-6 ||
        value.metadata.stream.warningCount !== value.stream.warnings.length ||
        Math.abs(value.metadata.stream.timeGranularityMs - value.stream.timeGranularityMs) > epsilon ||
    ...
    if (value.stream.points.length === 0) {
      if (stats.minFrequencyIndex !== null || stats.maxFrequencyIndex !== null ||
          stats.minIntensity !== null || stats.maxIntensity !== null || stats.meanIntensity !== null) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
...
export const exportDataSchema = z.object({
  ...
  downloadId: z.string().regex(/^[A-Za-z0-9._~-]+$/).optional(),
  contentType: z.string().regex(/^[A-Za-z0-9!#$%&*+.^_`|~-]+\/[A-Za-z0-9!#$%&*+.^_`|~-]+$/).optional()
}).strict();
...
export const batchDataSchema = z.object({
  ...
  const warningFiles = value.items.filter((item) => item.diagnostics.some((diagnostic) => diagnostic.severity === 'warning')).length;
  if (value.succeeded !== succeeded || value.rejected !== rejected || value.failed !== failed ||
      value.warningFiles !== warningFiles || value.cancelled !== (cancelled > 0)) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['items'],
  ...
  if (value.completed !== value.items.length || value.completed !== value.succeeded + value.rejected + value.failed + cancelled) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['completed'],
...
export const batchExportRequestSchema = batchRequestSchema.extend({
  items: z.array(batchInputSchema.extend({
...
export const pulseDiffSchema = z.object({
  equal: z.boolean(),
  ...
  const hasChanges = value.structural.length > 0 || value.metadata.length > 0 ||
    value.stream.length > 0 || value.text.length > 0;
...
const operationNameSchema = z.string().min(1).max(80).regex(/^[a-z][a-z0-9-]*$/);
...
const envelopeCommon = {
  ...
  operationId: z.string().regex(/^[A-Za-z0-9._~-]+$/).optional()
} as const;

const successfulEnvelopeSchema = z.object({
  ...envelopeCommon,
  status: z.literal('success'),
  result: z.unknown().refine((value) => value !== null && value !== undefined, 'Successful results must include result data.')
}).strict();
...
export function projectOperationResult<T, U>(
  ...
  const diagnostics = (Array.isArray(input.diagnostics) ? input.diagnostics : [])
    ...
    .map((parsed) => parsed.success ? parsed.data : {
  ...
  const safeDiagnostics = diagnostics.length > 0 ? diagnostics : input.status === 'success'
    ? diagnostics
    : [{
        code: 'PULSE_TASK_INVALID_TRANSITION',
        severity: 'error' as const,
  ...
  const operation = typeof input.operation === 'string' && /^[a-z][a-z0-9-]{0,79}$/.test(input.operation)
    ? input.operation
    : 'task';
  const common = {
  ...
  try {
    const result = input.status === 'success' && input.data !== null
...
export function projectBatchItem<T, U>(
  ...
  const safeStatus = input.status === 'success' || input.status === 'rejected' ||
    input.status === 'failed' || input.status === 'cancelled'
    ? input.status
    : 'failed';
  const safeDiagnostics = diagnostics.length > 0 ? diagnostics : safeStatus === 'success'
    ? diagnostics
    : [{
        code: 'PULSE_TASK_INVALID_TRANSITION' as const,
        severity: 'error' as const,
  ...
  try {
    return batchItemSchema.parse({
      ...
      status: safeStatus,
      diagnostics: safeDiagnostics,
      result: safeStatus === 'success' && input.data !== null
        ? project(input.data)
        : null
    });
  } catch {
