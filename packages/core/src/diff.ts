...
function entry(path: string, before: unknown, after: unknown): DiffEntry {
  return {
    path,
    before: typeof before === 'string' || typeof before === 'number' || typeof before === 'boolean'
      ? before
      : null,
    after: typeof after === 'string' || typeof after === 'number' || typeof after === 'boolean'
      ? after
      : null
  };
}
...
export function diffPulse(
  ...
  compareValue('globals.sectionRestIndex', before.globals.sectionRestIndex, after.globals.sectionRestIndex, structural);
  compareValue('globals.playbackSpeed', before.globals.playbackSpeed, after.globals.playbackSpeed, structural);
  compareValue('globals.frequencyBalanceIndex', before.globals.frequencyBalanceIndex, after.globals.frequencyBalanceIndex, structural);
  ...
  for (let sectionIndex = 0; sectionIndex < sectionCount; sectionIndex += 1) {
    ...
    compareValue(path + '.frequencyStartIndex', left.frequencyStartIndex, right.frequencyStartIndex, structural);
    compareValue(path + '.frequencyEndIndex', left.frequencyEndIndex, right.frequencyEndIndex, structural);
    ...
    compareValue(path + '.points.length', left.pulseElement.points.length, right.pulseElement.points.length, structural);
    ...
    for (let pointIndex = 0; pointIndex < pointCount; pointIndex += 1) {
      ...
      compareValue(path + '.points[' + pointIndex + '].strength', beforePoint.strengthDecimal, afterPoint.strengthDecimal, structural);
      compareValue(path + '.points[' + pointIndex + '].anchor', beforePoint.anchor, afterPoint.anchor, structural);
    }
  }
  ...
  compareValue('metadata.pulse.sectionCount', beforeMetadata.pulse.sectionCount, afterMetadata.pulse.sectionCount, metadata);
  compareValue('metadata.pulse.enabledSectionCount', beforeMetadata.pulse.enabledSectionCount, afterMetadata.pulse.enabledSectionCount, metadata);
  compareValue('metadata.pulse.disabledSectionCount', beforeMetadata.pulse.disabledSectionCount, afterMetadata.pulse.disabledSectionCount, metadata);
  compareValue('metadata.pulse.sourceDurationMs', beforeMetadata.pulse.sourceDurationMs, afterMetadata.pulse.sourceDurationMs, metadata);
  compareValue('metadata.pulse.effectiveDurationMs', beforeMetadata.pulse.effectiveDurationMs, afterMetadata.pulse.effectiveDurationMs, metadata);
  const metadataSectionCount = Math.min(beforeMetadata.sections.length, afterMetadata.sections.length);
  for (let sectionIndex = 0; sectionIndex < metadataSectionCount; sectionIndex += 1) {
    ...
    compareValue(path + '.targetDurationMs', left.targetDurationMs, right.targetDurationMs, metadata);
    compareValue(path + '.effectiveDurationMs', left.effectiveDurationMs, right.effectiveDurationMs, metadata);
    compareValue(path + '.repetitionCount', left.repetitionCount, right.repetitionCount, metadata);
    compareValue(path + '.pulseElementDurationMs', left.pulseElementDurationMs, right.pulseElementDurationMs, metadata);
    compareValue(path + '.pointCount', left.pointCount, right.pointCount, metadata);
  }
  compareValue('metadata.stream.timeGranularityMs', beforeMetadata.stream.timeGranularityMs, afterMetadata.stream.timeGranularityMs, metadata);
  compareValue('metadata.stream.sectionCount', beforeMetadata.stream.sectionCount, afterMetadata.stream.sectionCount, metadata);
  compareValue('metadata.stream.warningCount', beforeMetadata.stream.warningCount, afterMetadata.stream.warningCount, metadata);
  compareValue('metadata.stream.ruleVersion', beforeMetadata.stream.ruleVersion, afterMetadata.stream.ruleVersion, metadata);
  const beforeStats = beforeMetadata.stream.stats;
  const afterStats = afterMetadata.stream.stats;
  (['pointCount', 'totalDurationMs', 'minFrequencyIndex', 'maxFrequencyIndex', 'minIntensity', 'maxIntensity', 'meanIntensity'] as const)
    .forEach((key) => {
  ...
  return Object.freeze({
    equal: structural.length === 0 && metadata.length === 0 && stream.length === 0 && text.length === 0,
...
function compareStreams(
  ...
  if (before === null || after === null) {
    if (before !== after) entries.push(entry('stream', before?.digest ?? null, after?.digest ?? null));
    return;
  }
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (left.frequencyIndex !== right.frequencyIndex) {
      entries.push(entry('stream.points[' + index + '].frequencyIndex', left.frequencyIndex, right.frequencyIndex));
    }
    if (left.intensityDecimal !== right.intensityDecimal) {
      entries.push(entry('stream.points[' + index + '].intensity', left.intensityDecimal, right.intensityDecimal));
    }
