...
export function targetDurationMs(
  ...
  const unit = Number.isFinite(candidateRules.durationUnitMs) && candidateRules.durationUnitMs > 0
    ? candidateRules.durationUnitMs
    : DEFAULT_RULE_SET.durationUnitMs;
  const index = candidateSection !== null && Number.isFinite(candidateSection.durationIndex) && candidateSection.durationIndex >= 0
...
export function sectionTiming(
  ...
  const candidateRules = isRecord(rules) ? rules as RuleSet : DEFAULT_RULE_SET;
  const candidateSection = isRecord(section) ? section : null;
  const pointDuration = Number.isFinite(candidateRules.pointDurationMs) && candidateRules.pointDurationMs > 0
    ? candidateRules.pointDurationMs
    : DEFAULT_RULE_SET.pointDurationMs;
  const points = candidateSection !== null && isRecord(candidateSection.pulseElement) &&
    Array.isArray(candidateSection.pulseElement.points)
    ? candidateSection.pulseElement.points
    : [];
  const pointCount = Number.isSafeInteger(points.length)
    ? points.length
    : 0;
  ...
  const repetitionCount = Math.max(
    1,
    Math.ceil(target / Math.max(1, pulseElementDurationMs))
  );
...
export function frequencyAt(
  ...
  if (!isRecord(section) || !isRecord(section.pulseElement) || !Array.isArray(section.pulseElement.points)) return 0;
  ...
  switch (section.frequencyMode) {
    ...
    case 3:
      return section.frequencyStartIndex +
        (section.frequencyEndIndex - section.frequencyStartIndex) * x;
    case 4:
      return section.frequencyEndIndex +
        (section.frequencyStartIndex - section.frequencyEndIndex) * x;
    default:
...
): { readonly pointCount: number; readonly durationMs: number; readonly timings: readonly SectionTiming[] } {
  const timings = pulse.sections.map((section) => sectionTiming(section, rules));
  let pointCount = 0;
  let durationMs = 0;
  let selectedSeen = false;
  const speed = rules.speedDivisor && pulse.globals.playbackSpeed > 0
...
export function expandWaveform(
  ...
  const hasSections = pulse !== null && typeof pulse === 'object' &&
    Array.isArray((pulse as { readonly sections?: unknown }).sections);
  const canProject = hasSections && !hasBlockingErrors(validation.diagnostics) && timingRulesValid &&
  ...
  const streamDigest = stableDigest(encodeUtf8(JSON.stringify({
  ...
  const stream: WaveformStream = Object.freeze({
    ...
    warnings: Object.freeze(sortDiagnostics(diagnostics).filter((item) => item.severity !== 'error')),
    digest: streamDigest
  });
  return Object.freeze({
    stream,
...
export function sectionMetadata(
  ...
  const sourcePoints: SectionPointMetadata[] = section.pulseElement.points.map((point, index) => Object.freeze({
