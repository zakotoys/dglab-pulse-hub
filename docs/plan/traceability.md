...
| `git diff --check`                                                     | 通过                                                                                                                                                                                       |
| `release.yml` 静态检查                                                 | 已配置：tag 校验、完整质量门禁、GHCR API/Web 多架构镜像、Windows x64/ARM64 NSIS、macOS Intel/ARM64 DMG、四架构便携 ZIP、Release 附件和 SHA-256 校验和；GitHub hosted runner 尚未在本地执行 |
...
| Electron 发布打包        | M3/M5      | Electron Forge + electron-builder NSIS + bundled main/preload | package/make、sandbox preload、启动和目标平台验证                                     | In progress（本机 macOS arm64 package/make/启动已通过；CI 已配置 Windows x64/ARM64 NSIS、macOS Intel/ARM64 DMG 和四架构便携 ZIP；Windows/macOS、签名和安装升级仍未验证） |
| CI/CD 发布流水线         | M5         | `.github/workflows/release.yml`                               | tag/version 校验、质量门禁、GHCR、多架构桌面产物、Release 和校验和                    | In progress（四架构矩阵配置已完成；GitHub hosted runner 发布尚未执行）                                                                                                   |
