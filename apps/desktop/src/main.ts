...
function createWindow(): void {
  ...
  if (typeof loadFile === 'function') {
    void Promise.resolve(loadFile.call(mainWindow, join(currentDirectory, 'index.html')))
      .catch(() => undefined);
  }
...
function rejectedIpc(
  message: string,
  code: string = DIAGNOSTIC_CODES.EXPORT_BLOCKED
): unknown {
  return toOperationDto(operationResult('export', 'rejected', null, [
    ipcDiagnostic(code, 'error', 'export', message)
  ]));
}
          ...
          operation === 'diff' ? 'Diff operation was cancelled.' :
            operation === 'batch' ? 'Batch operation was cancelled.' :
              operation === 'render' ? 'Preview rendering was cancelled.' :
                operation === 'undo' ? 'Undo operation was cancelled.' :
                  operation === 'redo' ? 'Redo operation was cancelled.' : 'Edit operation was cancelled.'
    )
  ]));
}
...
function failedIpc(
  ...
  return toOperationDto(operationResult(operation, 'failed', null, [
    ipcDiagnostic(code, 'error', stage, message)
  ]));
}
...
function rejectedOperation(
  ...
  return toOperationDto(operationResult(operation, 'rejected', null, [
    ipcDiagnostic(code, 'error', stage, message)
  ]));
}
...
function digestPayload(payload: unknown): string | null {
  if (!plainObject(payload) || typeof payload.sourceDigest !== 'string' || !/^[0-9a-f]{16}$/i.test(payload.sourceDigest)) return null;
...
function recordCurrentSnapshot(): void {
  ...
  historySnapshots = [
    ...historySnapshots.slice(0, historyCursor + 1),
    next
  ];
...
function updateCurrentSnapshot(content: Uint8Array, path: string, dirty: boolean): OperationResult<InspectData> {
  const inspected = inspectPulse(content, {
    input: { displayName: sanitizeDisplayName(path.split(/[\\/]/).pop() ?? 'pulse.pulse'), bytes: content.byteLength }
  });
...
async function readSelectedBatchFiles(
...
): Promise<{ readonly inputs: readonly BatchInput[]; readonly paths: readonly string[] } | { readonly cancelled: true } | { readonly error: unknown }> {
  ...
  for (const path of selected.filePaths) {
    ...
    if (read.status === 'success' && read.data !== null) {
      ...
      inputs.push({ displayName: sanitizeDisplayName(path.split(/[\\/]/).pop() ?? 'pulse.pulse'), content: new Uint8Array(), diagnostics: read.diagnostics });
    }
  }
...
function validAssistPayload(value: Record<string, unknown>): AssistOptions | null {
  const fields = ['sectionIndex', 'startPointIndex', 'endPointIndex', 'startStrength', 'endStrength'] as const;
  ...
  if (![sectionIndex, startPointIndex, endPointIndex].every((number) => Number.isSafeInteger(number) && number >= 0) ||
      startPointIndex >= endPointIndex || ![startStrength, endStrength].every((number) => number >= 0 && number <= 100)) return null;
  return { sectionIndex, startPointIndex, endPointIndex, startStrength, endStrength, reviewed: true };
}
...
function parseIpcEditCommand(value: Record<string, unknown>): EditCommand | null {
  if (!onlyKeys(value, ['kind', 'sectionIndex', 'pointIndex', 'value', 'startIndex', 'endIndex', 'atIndex', 'anchor']) ||
      typeof value.kind !== 'string' || !validIpcIndex(value.sectionIndex)) return null;
  ...
  switch (value.kind) {
    ...
    case 'anchor':
      return validIpcIndex(value.pointIndex) && (value.value === 0 || value.value === 1)
        ...
        : null;
    case 'frequency':
      return validIpcIndex(value.startIndex) && validIpcIndex(value.endIndex) &&
        value.startIndex <= 83 && value.endIndex <= 83
        ? { kind: 'frequency', sectionIndex, startIndex: value.startIndex, endIndex: value.endIndex }
        : null;
    case 'duration':
    ...
    case 'add-point': {
      ...
      if (typeof strength !== 'number' || !Number.isFinite(strength) || strength < 0 || strength > 100 ||
          (anchor !== 0 && anchor !== 1) || (atIndex !== undefined && !validIpcIndex(atIndex))) return null;
...
function registerIpc(): void {
    ...
    try {
      ...
      if (inspected.status === 'success' && inspected.data !== null) {
        if (requestSequence === openSequence) {
          resetDocumentHistory(Object.freeze({
            digest: inspected.data.sourceDigest,
  ...
  ipcMain.handle('pulse:inspect-current', async (event) => {
    ...
    if (snapshot === null) return rejectedOperation('inspect', 'No pulse document is open.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    try {
      const inspected = inspectPulse(snapshot.content, {
        input: { displayName: sanitizeDisplayName(snapshot.path.split(/[\\/]/).pop() ?? 'pulse.pulse'), bytes: snapshot.content.byteLength }
      });
      return toOperationDto(inspected);
    } catch {
      return failedIpc('inspect', DIAGNOSTIC_CODES.ADAPTER_READ, 'adapter', 'The current pulse could not be inspected.');
    }
  });
  ...
  ipcMain.handle('pulse:decode-qr', async (event, payload: unknown) => {
    ...
    if (!plainObject(payload) || !onlyKeys(payload, ['text']) || typeof payload.text !== 'string') {
      return rejectedOperation('qr-decode', 'QR content must be text.', DIAGNOSTIC_CODES.QR_PREFIX, 'qr');
    }
    try {
      ...
      return toOperationDto(operationResult(
        'inspect',
        inspected.status,
        inspected.data,
        [...decoded.diagnostics, ...inspected.diagnostics]
      ));
    } catch {
      return failedIpc('inspect', DIAGNOSTIC_CODES.ADAPTER_READ, 'adapter', 'QR content could not be decoded.');
    }
  });

  ipcMain.handle('pulse:edit', async (event, payload: unknown) => {
    if (!trustedSender(event)) throw new Error('Untrusted IPC sender.');
    if (!plainObject(payload) || !onlyKeys(payload, ['sourceDigest', 'command'])) return rejectedOperation('edit', 'Edit request contains unsupported fields.', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic');
    const digest = digestPayload(payload);
    if (digest === null) return rejectedOperation('edit', 'Edit request must identify the opened source.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    const snapshot = currentSnapshot(digest);
    if (snapshot === null) return rejectedOperation('edit', 'The opened source is no longer available.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    if (!plainObject(payload.command)) return rejectedOperation('edit', 'Edit request must contain a command.', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic');
    const command = parseIpcEditCommand(payload.command);
    if (command === null) return rejectedOperation('edit', 'Edit command is invalid.', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic');
    try {
      ...
      if (inspected.status !== 'success' || inspected.data === null) {
        ...
        documentDirty = true;
        return failedIpc('edit', DIAGNOSTIC_CODES.EXPORT_ROUNDTRIP_MISMATCH, 'export', 'The edited pulse could not be re-inspected.');
      }
      recordCurrentSnapshot();
      return toOperationDto(edited);
    } catch {
      return failedIpc('edit', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic', 'The pulse edit could not be applied.');
    }
  });

  ipcMain.handle('pulse:assist', async (event, payload: unknown) => {
    if (!trustedSender(event)) throw new Error('Untrusted IPC sender.');
    if (!plainObject(payload) || !onlyKeys(payload, ['sourceDigest', 'sectionIndex', 'startPointIndex', 'endPointIndex', 'startStrength', 'endStrength', 'reviewed'])) return rejectedOperation('edit', 'Assist request contains unsupported fields.', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic');
    const digest = digestPayload(payload);
    if (digest === null) return rejectedOperation('edit', 'Assist request must identify the opened source.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    const snapshot = currentSnapshot(digest);
    if (snapshot === null) return rejectedOperation('edit', 'The opened source is no longer available.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    const options = validAssistPayload(payload);
    if (options === null) return rejectedOperation('edit', 'Assist requires explicit review and valid endpoints.', DIAGNOSTIC_CODES.EDIT_NOT_REVIEWED, 'semantic');
    try {
      ...
      if (inspected.status !== 'success' || inspected.data === null) {
        sourceSnapshot = snapshot;
        documentDirty = true;
        return failedIpc('edit', DIAGNOSTIC_CODES.EXPORT_ROUNDTRIP_MISMATCH, 'export', 'The assisted pulse could not be re-inspected.');
      }
      recordCurrentSnapshot();
      return toOperationDto(edited);
    } catch {
      return failedIpc('edit', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic', 'The assisted pulse could not be applied.');
    }
  });
  ...
  const registerHistoryHandler = (
    ...
    ipcMain.handle(channel, async (event, payload: unknown) => {
      ...
      if (!plainObject(payload) || !onlyKeys(payload, ['sourceDigest'])) {
        return rejectedOperation(operation, operation === 'undo'
          ? 'Undo request contains unsupported fields.'
          : 'Redo request contains unsupported fields.', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic');
      }
      const digest = digestPayload(payload);
      if (digest === null) {
        return rejectedOperation(operation, operation === 'undo'
          ? 'Undo request must identify the opened source.'
          : 'Redo request must identify the opened source.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
      }
      if (sourceSnapshot === null || originalSnapshot === null || historyCursor < 0) {
        return rejectedOperation(operation, 'No pulse history is available.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
      }
      if (sourceSnapshot.digest !== digest) {
        return rejectedOperation(operation, 'The opened source changed before the history operation completed.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
      }
      ...
      if (target === undefined) {
        return rejectedOperation(operation, operation === 'undo'
          ...
          : 'There is no later pulse snapshot to restore.', DIAGNOSTIC_CODES.TASK_INVALID_TRANSITION, 'task');
      }
      try {
        ...
        if (inspected.data.sourceDigest !== target.digest) {
          return failedIpc(operation, DIAGNOSTIC_CODES.TASK_INVALID_TRANSITION, 'task', 'The pulse history snapshot is inconsistent.');
        }
        selectHistorySnapshot(targetIndex);
        return response;
      } catch {
        return failedIpc(operation, DIAGNOSTIC_CODES.ADAPTER_READ, 'adapter', operation === 'undo'
  ...
  ipcMain.handle('pulse:diff', async (event, payload: unknown) => {
    if (!trustedSender(event)) throw new Error('Untrusted IPC sender.');
    if (!plainObject(payload) || !onlyKeys(payload, ['sourceDigest'])) return rejectedOperation('diff', 'Diff request contains unsupported fields.', DIAGNOSTIC_CODES.EDIT_VALUE, 'semantic');
    const digest = digestPayload(payload);
    if (digest === null) return rejectedOperation('diff', 'Diff request must identify the opened source.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    const snapshot = currentSnapshot(digest);
    if (snapshot === null) return rejectedOperation('diff', 'The opened source is no longer available.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    try {
      const selected = await dialog.showOpenDialog({
        properties: ['openFile'],
        filters: [{ name: 'Pulse files', extensions: ['pulse', 'txt'] }]
      ...
      return failedIpc('diff', DIAGNOSTIC_CODES.ADAPTER_READ, 'adapter', 'The comparison pulse could not be read.');
    }
  });
  ...
  ipcMain.handle('pulse:batch-inspect', async (event) => {
    ...
    try {
      ...
      if ('error' in selected) return failedIpc('batch', DIAGNOSTIC_CODES.ADAPTER_READ, 'adapter', 'Batch files could not be read.');
      return toOperationDto(await inspectBatch(selected.inputs));
    } catch {
      return failedIpc('batch', DIAGNOSTIC_CODES.ADAPTER_READ, 'adapter', 'Batch files could not be read.');
    }
  });

  ipcMain.handle('pulse:batch-export', async (event, payload: unknown) => {
    if (!trustedSender(event)) throw new Error('Untrusted IPC sender.');
    if (!plainObject(payload) || !onlyKeys(payload, ['mode', 'overwrite'])) return rejectedOperation('batch', 'Batch export request contains unsupported fields.', DIAGNOSTIC_CODES.EXPORT_UNSUPPORTED_MODE, 'export');
    const value = payload;
    const mode = value.mode === undefined ? undefined : value.mode;
    if (mode !== undefined && mode !== 'source' && mode !== 'canonical') return rejectedOperation('batch', 'Batch export mode is invalid.', DIAGNOSTIC_CODES.EXPORT_UNSUPPORTED_MODE, 'export');
    if (value.overwrite !== undefined && typeof value.overwrite !== 'boolean') {
      return rejectedOperation('batch', 'Batch export overwrite flag must be boolean.', DIAGNOSTIC_CODES.EXPORT_UNSUPPORTED_MODE, 'export');
    }
    ...
    try {
      ...
      if (directory.canceled || directory.filePaths[0] === undefined) return cancelledIpc('batch', exported.diagnostics);
      ...
      const combined = operationResult('batch', batchStatus, batchStatus === 'success' ? {
        ...exported.data,
        succeeded,
        rejected,
        failed,
        items: Object.freeze(items)
      } : null, items.flatMap((item) => item?.diagnostics ?? []));
      return toOperationDto(combined);
    } catch {
      return failedIpc('batch', DIAGNOSTIC_CODES.ADAPTER_WRITE, 'adapter', 'Batch export could not be completed.');
    }
  });
  ...
  ipcMain.handle('pulse:render-preview', async (event, payload: unknown) => {
    ...
    if (!plainObject(payload) || !onlyKeys(payload, ['sourceDigest', 'displayName', 'format'])) {
      return rejectedOperation('render', 'Preview request contains unsupported fields.', DIAGNOSTIC_CODES.EXPORT_UNSUPPORTED_FORMAT, 'export');
    }
    const sourceDigest = payload.sourceDigest;
    if (typeof sourceDigest !== 'string' || !/^[0-9a-f]{16}$/i.test(sourceDigest)) {
      return rejectedOperation('render', 'Preview request must identify the opened source.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    }
    const snapshot = currentSnapshot(sourceDigest);
    if (snapshot === null) {
      return rejectedOperation('render', 'The opened source is no longer available.', DIAGNOSTIC_CODES.EXPORT_SOURCE_UNAVAILABLE, 'adapter');
    }
    const format = payload.format;
    if (format !== 'svg' && format !== 'png' && format !== 'jpg') {
      return rejectedOperation('render', 'Preview format is not supported.', DIAGNOSTIC_CODES.EXPORT_UNSUPPORTED_FORMAT, 'export');
    }
    if (payload.displayName !== undefined && typeof payload.displayName !== 'string') {
      return rejectedOperation('render', 'Preview display name must be text.', DIAGNOSTIC_CODES.EXPORT_UNSUPPORTED_FORMAT, 'export');
    }
    try {
      const inspected = inspectPulse(snapshot.content, {
        input: {
      ...
      if (inspected.status !== 'success' || inspected.data?.stream === null || inspected.data?.stream === undefined) {
        return toOperationDto(operationResult('render', inspected.status === 'success' ? 'rejected' : inspected.status, null, inspected.diagnostics));
      }
      let image: ReturnType<typeof renderPreviewImage>;
      try {
        image = renderPreviewImage(inspected.data.stream, format);
      } catch {
        return failedIpc('render', DIAGNOSTIC_CODES.EXPORT_BLOCKED, 'export', 'Preview could not be encoded.');
      }
      ...
      return toOperationDto(operationResult('render', 'success', {
      ...
      return failedIpc('render', DIAGNOSTIC_CODES.ADAPTER_WRITE, 'adapter', 'Unable to save the preview image.');
    }
  });
    ...
    try {
      const exported = exportPulse(snapshot.content, {
        ...
        mode: requestedMode === undefined
          ? (format === 'pulse-text' ? (dirty ? 'canonical' : 'source') : undefined)
          : requestedMode,
  ...
  ipcMain.handle('pulse:mark-dirty', async (event, payload: unknown) => {
    ...
    if (!plainObject(payload) || typeof payload.dirty !== 'boolean') {
      return rejectedIpc('Dirty state must be a boolean.', DIAGNOSTIC_CODES.TASK_INVALID_TRANSITION);
    }
    const derivedDirty = sourceSnapshot !== null && originalSnapshot !== null &&
      sourceSnapshot.digest !== originalSnapshot.digest;
